...
OpenCode için eklenti koleksiyonu. Altı eklenti içerir — **`opencode-context-saver` (DHS PTC-mode)** context tasarrufu, **`opencode-build-tracker`** build yaşam döngüsü kancaları, **`opencode-truncation-noticer`** read-tool kırpma bildirimi, **`opencode-cpu-liveness`** CPU-izleme disclosure'ı, **`opencode-settle-noticer`** biten build'i sorulmadan bildirme ve **`opencode-hbmon`** hbmon custom tool'ları (turn-içi ajan wakeup). Artı MCP server **`bash`** (`bash_safe`/`bash_raw` TUI adları; server-içi `safe`/`raw`) ve script seti **`scripts/build-mon.mjs`** (push/event build monitörü) + **`scripts/cpu-liveness-probe/`**.
...
| **bash** (MCP) | `plugins/mcp-bash-tools/` | `bash_safe` (otomatik kırpılan) + `bash_raw` (tam çıktı) | — |
...
`pluginOptions["opencode-plugins"]` (altısına ortak; `enabled:false`
altısını birden kapatır).
